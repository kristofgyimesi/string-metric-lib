...
object DamerauLevenshtein {
  ...
  /** Computes normalized Damerau-Levenshtein string similarity value between two strings.
    ...
    * @param substitutionCosts A HashMap that sets the cost for `substituting` specific characters.
    *                          Each key is a tuple of characters, and the value is the cost of substituting the first
    *                          character for the second character. The values should be set between 0.0 and 1.0.
    * @param transpositionCosts A HashMap that sets the cost for `transposing` specific characters.
    *                           Each key is a tuple of characters, and the value is the cost of
    *                           transposing these characters, if they are next to each other.
    *                           The values should be set between 0.0 and 1.0.
    * @return The normalized Damerau-Levenshtein string similarity between `str1` and `str2`.
    */
  def similarity(
      str1: String,
      str2: String,
  ...
  /** Computes the Damerau-Levenshtein distance between two strings based on symmetric cost maps.
    * The operation costs can be parameterized.
    *
    * @param str1 The first string.
    * @param str2 The second string.
    ...
    val finalRow = (1 to len1).foldLeft(initialRow, initialRow) { case ((twoRowsBack, previousRow), i) =>
      ...
      (1 to len2).foreach { j =>
        ...
        if (s1CurrChar == s2CurrChar) {
          ...
          if (i > 1 && j > 1 && s1CurrChar == s2(j - 2) && s1(i - 2) == s2CurrChar) {
            val transCost = transpositionCosts
                              .getOrElse((s1(i - 2), s1CurrChar),
                                transpositionCosts.getOrElse((s1CurrChar, s1(i - 2)), 1.0))
            currentRow(i) = math.min(currentRow(i), twoRowsBack(i - 2) + transCost)
          }
        }
      }
  ...
  /** Computes normalized Damerau-Levenshtein string similarity value between two strings based on symmetric cost maps.
  ...
  def symmetricSimilarity(
    ...
    val maxLength = Seq(s1.length, s2.length).max
    val distance = symmetricDistance(s1, s2, deletionAndInsertionCosts, substitutionCosts, transpositionCosts)
